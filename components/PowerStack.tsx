  ...
  return (
    <div className="space-y-2.5">
      {layers.map((l) => {
        ...
        return (
          <div key={l.name} className="flex items-center gap-3">
            <div className="w-28 shrink-0 text-right text-xs text-slate-700">{l.name}</div>
            <div className="relative h-7 flex-1 overflow-hidden rounded-md border border-border bg-panel-2">
              ...
              <div className="relative flex h-full items-center justify-between px-2 text-[11px]">
                <span className="tabular text-sky-700">{l.used.toFixed(2)} MW</span>
      ...
      <p className="pt-1 text-xs text-muted">
        ...
        <span className="tabular text-slate-800">{peakIt.toFixed(2)} MW</span> against a 10 MW grid
